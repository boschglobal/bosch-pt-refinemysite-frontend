...
@Component({
    selector: 'ss-daycard-reason-multiple',
    templateUrl: './day-card-multiple-reason.component.html',
})
export class DayCardMultipleReasonComponent implements OnInit, OnDestroy {
    ...
    public handleSubmit(reason: RfvKey) {
        const dayCardIds = this._modalService.currentModalData.dayCardIds;

        this._store.dispatch(new DayCardActions.Cancel.All({
            reason,
            dayCardIds,
        }));
    }
