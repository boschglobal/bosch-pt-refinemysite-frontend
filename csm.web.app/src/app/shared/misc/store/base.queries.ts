...
import {ResourceLink} from '../api/datatypes/resource-link.datatype';
 ...
 *
 */
export abstract class BaseQueries<R, S, LL = {[key: string]: ResourceLink}> {
    ...
    public getCurrentItem(): (state: State) => R {
        return (state: State) => {
            return this._getSlice(state).items
    ...
    public getCurrentItemId(): (state: State) => string {
        return (state: State) => {
            return this._getSlice(state).currentItem.id;
        };
    }
    ...
    public getList(): (state: State) => R[] {
        return (state: State) => {
            return this._getSlice(state).list.ids
    ...
    public getCurrentPage(): (state: State) => R[] {
        return (state: State) => {
            const page: number = this._getSlice(state).list.pagination.page;
    ...
    public getCurrentItemRequestStatus(): (state: State) => RequestStatusEnum {
        return (state: State) => {
            return this._getSlice(state).currentItem.requestStatus;
        };
    }
    ...
    public getListRequestStatus(): (state: State) => RequestStatusEnum {
        return (state: State) => {
            return this._getSlice(state).list.requestStatus;
        };
    }
    ...
    /**
     * @description Retrieves query function to get list sort data
     * @returns {(state: State) => SorterData}
     */
    public getListSort(): (state: State) => SorterData {
        return (state: State) => {
            return this._getSlice(state).list.sort;
        };
    }

    /**
     * @description Retrieves query function to get list pagination data
     * @returns {(state: State) => SorterData}
     */
    public getListPagination(): (state: State) => SorterData {
    ...
    public getListLinks(): (state: State) => LL {
        return (state: State) => {
            return this._getSlice(state).list._links || {};
        };
    }
