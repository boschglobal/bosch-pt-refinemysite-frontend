...
export class ApiUrlBuilder {
    ...
    public withCursorPaginationAfter(limit: number, after?: string): ApiUrlBuilder {
        this._queryParams.set('limit', limit);

        if (after) {
            this._queryParams.set('after', encodeURIComponent(after));
        }

        return this;
    }

    public withCursorPaginationBefore(limit: number, before?: string): ApiUrlBuilder {
        this._queryParams.set('limit', limit);

        if (before) {
            this._queryParams.set('before', encodeURIComponent(before));
        }
